...
export class CompountInterestTableComponent implements OnInit {
  ...
  public randomize(): void {
    for (let i = 0; i < this.lineChartData.datasets.length; i++) {
      for (let j = 0; j < this.lineChartData.datasets[i].data.length; j++) {
        ...
        CompountInterestTableComponent.generateNumber(i);
      }
    }
    this.chart?.update();
  }
  ...
  public pushOne(): void {
    this.lineChartData.datasets.forEach((x, i) => {
      const num = CompountInterestTableComponent.generateNumber(i);
      x.data.push(num);
    });
